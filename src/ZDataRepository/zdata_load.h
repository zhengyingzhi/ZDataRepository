 ...
 * return   ZDATA_Ok-成功 
 */
typedef int(*zdata_process_pt)(void* result, void* context, int number, const char* data, int length);
 ...
 * return   指针地址
 */
typedef void*(*zdata_alloc_pt)(void* context, int size);
...
typedef struct  
{
    zdata_process_pt    proc;       // 可以使用系统默认的
    ...
    zdata_alloc_pt      alloc_ptr;  // 内存分配函数
}zdata_load_conf_t;
